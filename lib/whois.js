...
const torInfo = ip => {
    return new Promise((resolve, reject) => {
        ...
        const host = reverseIP(ipaddr).concat('.', TORServer);
...
const extractIPGen = function * (str) {
    ...
    for (let re of EXTRACT_IP_REGEXP) {
        ...
        while ((match = re.exec(str)) !== null) {
            // found IP that we has not found earlier, save and return it
...
const bgpInfo = host => {
    return new Promise((resolve, reject) => {
        ...
        if (net.isIPv4(ip))
            bgpRequestHost = reverseIP(ip).concat('.', bgpServer4);

        else if (net.isIPv6(ip))
            bgpRequestHost = reverseIP(ip).concat('.', bgpServer6);
