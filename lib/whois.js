 ...
 */
const geoInit = async (dbPath, files) => {
    ...
    // init ip2location
    ip2loc.IP2Location_init(path.join(dbPath, files.ip2location));

    // init GeoIP2
    maxMind = await mmdbReader.open(path.join(dbPath, files.maxMind));
...
const geoInfo = async host => {
    ...
    const data = await Promise.all([
        Promise.resolve(maxMind.city(host)),
    ...
    if (data[0] !== null) {
        // country
        result.country.push(     data[0].country.names.en);
        result.country_code.push(data[0].country.isoCode );

        result.country_ru = data[0].country.names.ru;

        // region
        if (Array.isArray(data[0].subdivisions) && data[0].subdivisions.length > 0) {
            result.region.push(data[0].subdivisions[0].names.en);
        ...
        // city
        if (('city' in data[0].city) && ('names' in data[0].city)) {
            result.city.push(data[0].city.names.en);
        ...
        if (typeof data[0].location.time_zone !== 'undefined') {
            result.timezone = data[0].location.time_zone;
        }
    }

    // fill from 'IP2Location' DB
    if (data[1] !== null) {
        result.country.push(     data[1].country_long );
        result.country_code.push(data[1].country_short);
        result.region.push(      data[1].region       );
        result.city.push(        data[1].city         );
...
const geoUpdate = async (dbPath, token) => {
    ...
    const TEST = {
        URL        : 'https://xinit.lo/IP2LOCATION-LITE-DB5.IPV6.BIN.ZIP',
        FILENAME   : path.join(dbPath, 'IP2LOCATION-LITE-DB5.IPV6.zip'),
        BASENAME   : path.join(dbPath, 'IP2LOCATION-LITE-DB5.IPV6.BIN'),
        COMPRESSION: 'zip'
    };
    const TEST2 = {
        URL        : 'https://xinit.lo/GeoLite2-City.tar.gz',
        FILENAME   : path.join(dbPath, 'GeoLite2-City.tar.gz'),
        BASENAME   : path.join(dbPath, 'GeoLite2-City.mmdb'),
        COMPRESSION: 'gzip'
    };
    */
    ...
    const promises = [MAXMIND, IP2LOCATON].map(DB => new Promise((resolve, reject) => {
...
const bgpInfo = async host => {
    ...
    let response = await dns.resolve(bgpRequestHost, 'TXT');
